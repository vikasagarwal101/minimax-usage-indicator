...
def _get_css_provider():
    ...
    if _shared_css_provider is None:
        ...
        _shared_css_provider.load_from_data(b"""
            .stat-frame {
                padding: 14px 18px;
                border-radius: 8px;
...
class DetailWindow(Gtk.ApplicationWindow):
    ...
    def update_data(self, d):
        ...
        if d["interval_total"] > 0:
            ...
            W["int_details"].set_label(f"Used: {int_pct}% of 5h window (count not exposed)")
        else:
        ...
        if d["weekly_total"] > 0:
            W["wk_lbl"].set_label(f"{wk_pct}% Used" if wk_pct is not None else "--% Used")
        ...
        for m in d.get("all_models", []):
            ...
            if m["interval_total"] > 0 and m.get("interval_remains") is not None:
                ...
                det_str = f"Interval: {i_pct}% used (count not exposed)"
            else:
                det_str = "Interval: --"
